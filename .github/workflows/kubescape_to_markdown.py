...
def convert_kubescape_json_to_markdown_resource_centric(json_file):
    """
    Reads a Kubescape JSON output file, uses jq to transform it,
    and converts it into a Markdown table (resource-centric).

    Args:
        json_file (str): The path to the Kubescape JSON file.

    Returns:
        str: A Markdown formatted table of the Kubescape scan results (resource-centric).
    """
    try:
    ...
    # jq script to extract and transform the data (resource-centric)
    jq_script = """
    {
...
if __name__ == "__main__":
    ...
    markdown_output = convert_kubescape_json_to_markdown_resource_centric(json_file_path)
